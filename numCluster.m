function varargout = numCluster(data, varargin)
...
%         since our data are usually high-dimensional, default values are
%         set to [1 1.5, 2].
...
%         (True/False).
% Output: numbClust: no. of clusters detected with each specified value of 
%         transformation power "y".
%         y: corresponding values of "y".
% 
...
for i = 1:length(y)
    ...
    numClust(i) = K(jumps(i,:) == max(jumps(i,:)));
    ...
    % Report maximum jump
    if (printresults)
        disp(['No. clusters: ',num2str(numClust(i)),' @ Y = ' ,num2str(y(i))]);
    end
end

switch nargout
    case 1
        varargout{1} = numClust;
    case 2
        varargout{1} = numClust;
        varargout{2} = y;
end
