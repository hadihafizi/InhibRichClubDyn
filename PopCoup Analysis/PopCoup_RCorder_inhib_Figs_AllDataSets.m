...
nonRich_idx_all = cell(length(dataSets), 1);
richness_nr_all = cell(length(dataSets), 1);
Coup_nr_r_norm_all = cell(length(dataSets), 1);
for i = 1:length(dataSets)

    % E/I and RC indices
    load([dataSets{i},'\PDF_NoSpur_thr45_',dataSets{i},'.mat'])
    load([dataSets{i},'\wgts_1_16ms.mat'])
    W = PDF_cor.*wgt;oute = sum(W,2);inte = sum(W,1);tote = inte + oute';
    [A, B] = sort(tote,'descend');
    num_rich = ceil(0.2*length(A));
    Rich_idx = B(1:num_rich);
    nonRich_idx = B(num_rich+1:end);
    nonRich_idx_all{i} = nonRich_idx;
    richness_r = [richness_r, A(1:num_rich)];
    richness_nr = [richness_nr, A(num_rich+1:end)];
    richness_nr_all{i} = richness_nr;
    exc_thresh = .9;
    inh_thresh = .9;
    %in-vitro
%     load(['C:\Users\mhafizi\Box Sync\Research\Beggs Lab\Projects\InhibID\Ian Stevenson lab\Neuron Type Analysis\Neuron Type Analysis\in vitro\DataSet', num2str(EIdatasets(i)), '.mat']);
    %in-vivo
    load(['C:\Users\mhafizi\Box Sync\Research\Beggs Lab\Projects\InhibID\Ian Stevenson lab\Neuron Type Analysis\Neuron Type Analysis\in vivo\unitactivity_Mouse ', num2str(dataSets{i}), '.mat']);
...
%     load(['PopCoupAll_', dataSets{i}, '_cor_toteRC.mat'])
    ...
    interval = (size(Coup_r_r, 2) - 1)/2;
    [Coup_r_r, Coup_r_nr, Coup_nr_r, Coup_nr_nr] = PopCoup_Figs(Coup_r_r, Coup_r_nr, Coup_nr_r, Coup_nr_nr, dataSets{i}, 0);
    Coup_r_r_norm = [Coup_r_r_norm;Coup_r_r(:, interval + 1)];
    Coup_r_e_norm = [Coup_r_e_norm;Coup_r_r(R_E,:)];
    Coup_r_i_norm = [Coup_r_i_norm;Coup_r_r(R_I,:)];
    Coup_nr_nr_norm = [Coup_nr_nr_norm;Coup_nr_nr(:, interval + 1)];
    Coup_nr_e_norm = [Coup_nr_e_norm;Coup_nr_nr(NR_E,:)];
    Coup_nr_i_norm = [Coup_nr_i_norm;Coup_nr_nr(NR_I,:)];
    Coup_r_nr_norm = [Coup_r_nr_norm;Coup_r_nr(:, interval + 1)];
    Coup_rnr_e_norm = [Coup_rnr_e_norm;Coup_r_nr(R_E,:)];
    Coup_rnr_i_norm = [Coup_rnr_i_norm;Coup_r_nr(R_I,:)];
    Coup_nr_r_norm = [Coup_nr_r_norm;Coup_nr_r(:, interval + 1)];
    Coup_nr_r_norm_all{i} = Coup_nr_r_norm;
    Coup_nrr_e_norm = [Coup_nrr_e_norm;Coup_nr_r(NR_E,:)];
    Coup_nrr_i_norm = [Coup_nrr_i_norm;Coup_nr_r(NR_I,:)];
end
%% 
parsave('NR_ZeroLagPopCoup_toRichClub.mat',{'nonRich_idx_all', 'richness_nr_all', 'Coup_nr_r_norm_all'}, {nonRich_idx_all, richness_nr_all, Coup_nr_r_norm_all})

%% PopCoup vs Richness

figure; 
semilogx(richness_r, Coup_r_r_norm, 'r.'); hold on
semilogx(richness_r, Coup_r_nr_norm, 'g.');
semilogx(richness_nr, Coup_nr_r_norm, 'bx');
semilogx(richness_nr, Coup_nr_nr_norm, 'kx');
legend('RR', 'RNR', 'NRR', 'NRNR', 'Location', 'best')
xlabel('Richness (TE)')
ylabel('Zero-lag PopCoup')
...
[recount, recenter] = hist(Coup_r_e_norm(:,interval + 1),bnedg); 
[ricount, ricenter] = hist(Coup_r_i_norm(:,interval + 1),bnedg); 
[nrnrcount, nrnrcenter] = hist(Coup_nr_nr_norm(:,interval + 1),bnedg); 
[nrecount, nrecenter] = hist(Coup_nr_e_norm(:,interval + 1),bnedg); 
[nricount, nricenter] = hist(Coup_nr_i_norm(:,interval + 1),bnedg); 
...
%% R-R and NR-NR
ax = figure; hold on
% rr = histogram(Coup_r_r_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);
re = histogram(Coup_r_e_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);
ri = histogram(Coup_r_i_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);
% nrnr = histogram(Coup_nr_nr_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);
nre = histogram(Coup_nr_e_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);
nri = histogram(Coup_nr_i_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);

legend('RE-R','RI-R','NRE-NR','NRI-NR');
title('Population Coupling Distributions', 'FontSize',16)
xlabel('Population Coupling [au]');
ylabel('Probability');
...
%% R-NR and NR-R
ax = figure(); hold on
% rnr = histogram(Coup_r_nr_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);
rnre = histogram(Coup_rnr_e_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);
rnri = histogram(Coup_rnr_i_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);
% nrr = histogram(Coup_nr_r_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);
nrre = histogram(Coup_nrr_e_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);
nrri = histogram(Coup_nrr_i_norm,bnedg, 'normalization', 'Probability', 'DisplayStyle','stairs', 'LineWidth', 2);

legend('RE-NR','RI-NR','NRE-R','NRI-R');
title('Population Coupling Distributions', 'FontSize',16)
xlabel('Population Coupling [au]');
ylabel('Probability');
set(gca,'tickDir','out','color','none','fontname','arial','linewidth',1,'fontsize',12,'xscale','log'); box off;
...
figure; hold on
histogram(driver_r_e, 'Normalization', 'Probability');
histogram(driver_r_i, 'Normalization', 'Probability');
histogram(driver_nr_e, 'Normalization', 'Probability');
histogram(driver_nr_i, 'Normalization', 'Probability');
% histogram(driver_rnr_e, 'Normalization', 'Probability');
% histogram(driver_rnr_i, 'Normalization', 'Probability');
% histogram(driver_nrr_e, 'Normalization', 'Probability');
% histogram(driver_nrr_i, 'Normalization', 'Probability');
xlabel('(AUC_{pre}-AUC_{post})/(AUC_{pre}+AUC_{post})')
ylabel('Probability')
