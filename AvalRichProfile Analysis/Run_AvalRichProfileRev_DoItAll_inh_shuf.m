...
%% Make spike-swapped versions of asdf files for each data set

% for i = 1:length(dataSet)
%     for shuf = 1:20
%         load([dataSet{i}, '/asdf.mat'])
%         timeraster = ASDFToSparse(asdf_raw);
%         timeraster_swapped = spikeSwapper4(timeraster);
%         asdf_swapped = SparseToASDF(timeraster_swapped, 1);
%         save([dataSet{i}, '/asdf_swapped.mat'], 'asdf_swapped')
%     end
% end
...
%     parsave([dataSet{i},'/aval_list',dataSet{i},'_noUnID_bs',num2str(BinSize),'ms.mat'], {'str_aval'}, {str_aval});
    for shuf = 1%:20
        str_aval = load([dataSet{i},'/aval_list',dataSet{i},'shuf', num2str(shuf), '_bs',num2str(BinSize),'ms.mat']);
        ...
        parsave([dataSet{i},'\AvalRichProfile_',dataSet{i},suffix,'_shuf',num2str(shuf),'_bs',num2str(BinSize),'ms.mat'],{'AvalLen','rchnss','err'},{AvalLen,rchnss,err})
        parsave([dataSet{i},'\AvalRichProfile_',dataSet{i},suffix,'_shuf',num2str(shuf),'_bs',num2str(BinSize),'ms_RE.mat'],{'AvalLen','rchnss_re','err_re'},{AvalLen,rchnss_re,err_re})
        parsave([dataSet{i},'\AvalRichProfile_',dataSet{i},suffix,'_shuf',num2str(shuf),'_bs',num2str(BinSize),'ms_RI.mat'],{'AvalLen','rchnss_ri','err_ri'},{AvalLen,rchnss_ri,err_ri})
        parsave([dataSet{i},'\AvalRichProfile_',dataSet{i},suffix,'_shuf',num2str(shuf),'_bs',num2str(BinSize),'ms_NR.mat'],{'AvalLen','rchnss_nr','err_nr'},{AvalLen,rchnss_nr,err_nr})
        parsave([dataSet{i},'\AvalRichProfile_',dataSet{i},suffix,'_shuf',num2str(shuf),'_bs',num2str(BinSize),'ms_NRE.mat'],{'AvalLen','rchnss_nre','err_nre'},{AvalLen,rchnss_nre,err_nre})
        parsave([dataSet{i},'\AvalRichProfile_',dataSet{i},suffix,'_shuf',num2str(shuf),'_bs',num2str(BinSize),'ms_NRI.mat'],{'AvalLen','rchnss_nri','err_nri'},{AvalLen,rchnss_nri,err_nri})
    %%
    %
    ...
    savefig(H,[dataSet{i},'\AvalRichActEI_',dataSet{i},'_bs',num2str(BinSize),'ms',suffix,'_shuf',num2str(shuf)])
    saveas(H,[dataSet{i},'\AvalRichActEI_',dataSet{i},'_bs',num2str(BinSize),'ms_',suffix,'_shuf',num2str(shuf),'.jpg'])
    end
end